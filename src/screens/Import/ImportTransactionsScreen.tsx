...
const ImportTransactionsScreen: React.FC = () => {
  ...
  const handleImportCsv = async () => {
    try {
      ...
      // Use standard fetch API instead of deprecated expo-file-system
      ...
      Papa.parse(csvString, {
        ...
        complete: async (results) => {
          if (!results.data || results.data.length === 0) {
            Alert.alert("Error", "CSV is empty.");
            return;
          }

          const headers = results.meta.fields || [];
          const dateCol = headers.find(h => h.toLowerCase().includes('date')) || headers[0];
          const amtCol = headers.find(h => h.toLowerCase().includes('amount') || h.toLowerCase().includes('debit') || h.toLowerCase().includes('withdrawal')) || headers[1];
          const noteCol = headers.find(h => h.toLowerCase().includes('description') || h.toLowerCase().includes('narration') || h.toLowerCase().includes('note') || h.toLowerCase().includes('particulars')) || headers[2];
          ...
          for (const row of results.data as any[]) {
            const rawAmount = parseFloat((row[amtCol] || '').replace(/[^0-9.-]+/g, ''));
            if (isNaN(rawAmount) || rawAmount === 0) continue; 
            
            // Treat everything as absolute expense for now
            ...
            const note = (row[noteCol] || 'Imported Expense').substring(0, 100);

            let categoryId = defaultCategory?.id || null;
            let categoryName = defaultCategory?.name || 'Uncategorized';
            
            try {
              const prediction = await mlService.predictCategory(note);
              if (prediction) {
                const matched = categories.find(c => 
                  c.name.toLowerCase() === String(prediction.category).toLowerCase() || 
                  c.id === prediction.category
                );
                if (matched) {
          ...
          // Pass the data to the Review Screen
          navigation.navigate('ReviewImport' as never, { parsedData } as never);
        },
        error: (err: any) => {
  ...
  const handleViewSample = () => {
    Alert.alert("Sample CSV Format", "Date, Amount, Description\n2023-10-01, 150.50, Uber Ride\n2023-10-02, 500.00, Grocery Store");
  };
