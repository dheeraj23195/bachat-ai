...
async function checkBudgetAlertsForNewExpense(saved: Transaction) {
  try {
    ...
    for (const b of relevantBudgets) {
      ...
      const oldSpent = spentForThisBudget - saved.amount;
      
      if (spentForThisBudget <= 0 || b.limitAmount <= 0) continue;

      const newUsagePercent = (spentForThisBudget / b.limitAmount) * 100;
      const oldUsagePercent = (oldSpent / b.limitAmount) * 100;
      const scopeLabel = b.categoryId === null ? "overall" : "category";

      // 1. Did we just cross 100%?
      if (newUsagePercent >= 100 && oldUsagePercent < 100) {
        await sendPushNotification(
          "🚨 Budget Exceeded", 
          `You have exceeded your ${scopeLabel} budget!`
        );
      }
      // 2. Did we just cross the custom warning threshold?
      else if (newUsagePercent >= b.alertThresholdPercent && oldUsagePercent < b.alertThresholdPercent) {
        await sendPushNotification(
          "⚠️ Budget Alert", 
          `Your ${scopeLabel} budget is at ${newUsagePercent.toFixed(0)}% of the limit.`
        );
      }
    }
  } catch (e) {
...
const AddExpenseScreen: React.FC<Props> = () => {
  ...
  const handleSaveExpense = async () => {
    ...
    try {
      ...
      // 🔔 Check budgets and send local notifications (fire-and-forget)
      checkBudgetAlertsForNewExpense(saved).catch(err => 
        console.error("Budget check failed", err)
      );

      // Train ML after successful save (fire-and-forget; don't block UX)
      mlService.trainOnTransaction({
        id: saved.id,
        note: note || "",
        merchant: undefined,
        categoryId: categoryToSaveId,
